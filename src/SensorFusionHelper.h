...
lane_ahead IsLaneClear(int lane, double car_s, nlohmann::json sensor_fusion, int prev_size, float timestep)
{
    ...
    for (int i = 0; i < sensor_fusion.size(); i++)
    {
        ...
        if (d < (2+4*lane+2) && d > (2+4*lane-2))
        {
            ...
            {
                // if car is closer then update
                if (check_car_s - car_s < dist_to_car_ahead)
                {
...
vector<double> AveLaneSpeed(int car_s, nlohmann::json sensor_fusion, int prev_size, float timestep)
{
    ...
    for (int i = 0; i < sensor_fusion.size(); i++)
    {
        ...
        // Check cars that are within 50 metres of our car...
        double dist_to_car = check_car_s - car_s;
        if (dist_to_car < 50 && dist_to_car > 0)
        {
    ...
    // Check empty lanes - if all empty - favour left
    if (speedsLane0.empty()) speedsLane0.push_back(99.9);
    if (speedsLane1.empty()) speedsLane1.push_back(99.8);
    if (speedsLane2.empty()) speedsLane2.push_back(99.7);
    // return
...
{
    // safety window size
    float min_gap_behind = -10; //m
    float min_gap_ahead = 50; //m
